...
const UserViewLeft = ({ data }) => {
  ...
  useEffect(() => { 
    let flagCountSubmitted = store.jobs.data.filter( val => (val.prescriber.id == data.id && val.job_stage == "Feedback completed" ) )
    let flagCountSubmittedInRadius = store.jobs.data.filter( val => (val.prescriber.id == data.id && val.job_stage == "Feedback completed" && parseFloat(val.difference_location_doctor__c) < 1 && parseFloat(val.difference_location_doctor__c) > 0 ) )
            ...
            <CardContent>
              ...
              <Box sx={{ paddingTop: 2, paddingBottom: 2 }}>
                <Box sx={{ display: 'flex', marginBottom: 2.7 }}>
                  ...
                  <Typography variant='body2'>{data?.Speciality__c}</Typography>
                </Box>
                <Box sx={{ display: 'flex', marginBottom: 2.7 }}>
                  <Typography sx={{ marginRight: 2, fontWeight: 500, fontSize: '0.875rem' }}>Furosemide TRX:</Typography>
                ...
                <Box sx={{ display: 'flex' }}>
                  ...
                  <Typography variant='body2'>{data?.prescriber_address}</Typography>
                </Box>
              </Box>
            </CardContent>
